...
#[test]
fn archive_insert_is_idempotent() {
    ...
    let body = &f[..f.find("\n}\n").expect("fn ends")];
    assert!(
        body.contains("ON CONFLICT (event_id, attendee_id) DO NOTHING"),
        "the archive insert must be idempotent — the cron re-runs daily, and a \
         second copy of a deposit double-counts the money it exists to record"
    );
}
