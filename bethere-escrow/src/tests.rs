...
// ===========================================================================
// TEST: Rollover → Refund from target event
// Verifies the full attendee journey: deposit on source → check-in → rollover
// → refund from target. USDC should end up back in attendee's wallet.
// ===========================================================================
...
// ===========================================================================
// TEST: Double rollover rejection
// After first rollover succeeds, second attempt from same source should fail
// because source deposit is marked refunded=true.
// ===========================================================================
...
// ===========================================================================
// TEST: Rollover → Claim Forfeited on target
// Two attendees: A rolls over deposit, B deposits directly and no-shows.
// Organizer claims B's forfeited deposit — A's rolled-over deposit must be
// untouched.
// ===========================================================================
...
fn test_rollover_then_claim_forfeited_target() {
    ...
    // Pre-state: attendee A rolled over (checked_in=false, refunded=false on target)
    //            attendee B deposited directly (checked_in=false, refunded=false on target)
    //            target vault has 2 * DEPOSIT_AMOUNT
    //
    // But claim_forfeited requires !checked_in && !refunded. Attendee A's rollover
    // deposit has checked_in=false (correct — they haven't been checked in on target yet).
    // So the organizer COULD claim A's deposit too. This test verifies only B's is claimed.
...
// ===========================================================================
// TEST: Rollover → Close source event
// After rollover, source event should be closeable (vault empty, all refunded).
// Target event should still hold the rollover deposit.
// ===========================================================================
