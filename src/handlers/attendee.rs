...
use crate::models::api::{AttendeeResponse, RecentCheckIn, StatsResponse};
...
///
/// Query parameters:
/// - `status`: filter by approval status (approved, checked_in, all)
///
...
pub async fn get_attendee(
    ...
    let qr_image = attendee
        .qr_code_url
        .as_ref()
        .and_then(|url| crate::qr::generate_qr_base64(url).ok());
