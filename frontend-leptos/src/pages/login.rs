...
use crate::api;
use crate::auth::{get_url_error, handle_token_from_url, is_authenticated};

/// Login page component.
///
/// Shows a Google OAuth sign-in button and handles:
...
pub fn Login() -> impl IntoView {
    ...
    // On mount: check URL for token or error, check auth state
    Effect::new(move |_| {
        // Handle token from OAuth callback
        let had_token = handle_token_from_url();
        if had_token {
            log::info!("[login] token found in URL, redirecting to /staff");
        ...
        if is_authenticated() {
            log::info!("[login] already authenticated, redirecting to /staff");
            navigate("/staff", Default::default());
        }
    });

    /// Google SVG icon bytes
    fn google_icon() -> &'static str {
        r#"<svg viewBox="0 0 24 24" width="20" height="20">
