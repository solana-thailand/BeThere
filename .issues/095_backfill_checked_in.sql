-- Corrective backfill for `.issues/095`.
--
-- The first pass (.issues/091_backfill.sql) filtered `approval_status =
-- 'approved'` and so enrolled exactly half the people who attended. The other
-- half sit at `checked_in`, which is the NEXT state, not a rival one.
--
-- Safe to run on a database that has already had the first pass: every
-- statement is INSERT OR IGNORE against a unique key, so the already-enrolled
-- rows are untouched and no survey is queued twice.
...
-- Same reason as the first pass: `notification_registration` fires
-- unconditionally on enrolment insert and queues a "Registration saved" message
-- for events that ended months ago.
