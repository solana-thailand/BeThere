...
**Status:** deployed 2026-09-25 (prod version `4eb155ac`, git `04629a5`, PR #151).
Fixed in session `event-checkin-72`. On staging first (`aceb3d5a`): a rename onto a
taken slug returned 400 with the new message, and the write smoke passed. Prod had no collisions when checked (read-only query below), so
